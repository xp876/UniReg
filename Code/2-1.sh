...
# Run ONLY design 3p3p for GSE142696 (plan8).
# You can run 2-1.sh / 2-2.sh / 2-3.sh in parallel on the same node.
# Use JOBS_PER_DESIGN to limit per-design concurrency and avoid oversubscription.
...
# Inputs (override if your filenames/paths differ)
